...
func (dict *SimpleDict) PutIfAbsentWithLock(key string, val interface{}) int {
	return dict.PutIfAbsent(key, val)
}
func (dict *SimpleDict) PutIfExistsWithLock(key string, val interface{}) int {
	return dict.PutIfExists(key, val)
}
